...
enum class ConnectionCloseCode : u8 {
    Normal,
    ReceiveError,
    SendError
};
...
public:
    ...
    void init(
        std::function<void(ConnectionCloseCode)>&& on_closed,
        std::function<void(std::vector<std::byte>&&)>&& on_received);
    void open();
    void close(ConnectionCloseCode code);
...
private:
    ...
    std::function<void(ConnectionCloseCode)> _on_closed;
...
Connection<SocketType>::~Connection() {
    close(ConnectionCloseCode::Normal);
}
...
template <typename SocketType>
void Connection<SocketType>::close(ConnectionCloseCode code) {
...
void Connection<SocketType>::send(std::unique_ptr<OutMessage> message) {
    ...
    dispatch(_strand, [this, message = std::move(message)] -> boost::asio::awaitable<void> {
        ...
        if (ec) close(ConnectionCloseCode::SendError);
    });
}
...
boost::asio::awaitable<void> Connection<SocketType>::receive() {
    ...
    if (const auto [ec, _] = co_await async_read(
        ...
        close(ConnectionCloseCode::ReceiveError);
        co_return;
    }
